<template>
  <div class="min-h-screen bg-[#0b0b0f] text-gray-100 font-sans selection:bg-pink-500/30">
    <!-- Note: sidebar might be doubled if layout provides it, but following dashboard.vue pattern -->
    ...
    <main class="ml-20 p-6 md:p-8 max-w-[1600px] mx-auto pb-24">
      ...
      <div v-if="!isLoggedIn" class="mb-10 p-8 rounded-3xl bg-gradient-to-r from-purple-900/40 to-pink-900/40 border border-white/10 relative overflow-hidden">
        <div class="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20"></div>
        <div class="relative z-10 max-w-2xl">
          <h1 class="text-4xl md:text-5xl font-extrabold text-white mb-4 leading-tight">
            ยินดีต้อนรับสู่ <span class="text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-purple-400">P2UKAISER</span>
          </h1>
          <p class="text-gray-300 text-lg mb-6">
            แหล่งรวมสินค้าไอทีและแฟชั่นระดับพรีเมียม ประมูลสินค้าแบรนด์เนม และเป็นเจ้าของไอเทมสุดพิเศษได้แล้ววันนี้
          </p>
          <div class="flex flex-wrap gap-4">
            <NuxtLink to="/login" class="px-8 py-3 rounded-xl bg-white text-black font-bold hover:bg-gray-200 transition-colors shadow-lg shadow-white/10">
              เข้าสู่ระบบ
            </NuxtLink>
            <NuxtLink to="/register" class="px-8 py-3 rounded-xl bg-white/10 text-white font-bold hover:bg-white/20 transition-colors border border-white/10 backdrop-blur-md">
              สมัครสมาชิก
            </NuxtLink>
          </div>
        </div>
      ...
      <div v-else-if="filteredProducts.length" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-5">
        ...
        >
          <div class="aspect-[4/5] relative overflow-hidden bg-[#09090b]">
            ...
            <!-- Wishlist Button - Redirects if guest -->
...
import { useRouter, useRoute, navigateTo } from '#imports'; // implicit
